...
public class SwerveDriveKinematics {
		...
		if (!centerOfRotationMeters.equals(m_prevCoR)) {
			for (int i = 0; i < m_numModules; i++) {
				m_inverseKinematics.setRow(
						i * 2 + 0, 0, /* Start Data */ 1, 0, -m_modules[i].y() + centerOfRotationMeters.y());
				m_inverseKinematics.setRow(
						i * 2 + 1, 0, /* Start Data */ 0, 1, +m_modules[i].x() - centerOfRotationMeters.x());
			}
	...
	public Twist2d toTwist2d(SwerveModulePosition... wheelDeltas) {
		...
		for (int i = 0; i < m_numModules; i++) {
			...
			moduleDeltaMatrix.set(i * 2 + 1, module.distanceMeters * module.angle.sin());
		}
	...
	public static void desaturateWheelSpeeds(
		...
		for (SwerveModuleState SwerveModuleState : moduleStates) {
			SwerveModuleState.speedMetersPerSecond *= scale;
		}
	}
}
