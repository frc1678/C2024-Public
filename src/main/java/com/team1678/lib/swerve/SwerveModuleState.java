...
public class SwerveModuleState implements Comparable<SwerveModuleState> {
	...
	public boolean equals(Object obj) {
		if (obj instanceof SwerveModuleState) {
			...
			return Math.abs(other.speedMetersPerSecond - speedMetersPerSecond) < 1E-9 && angle.equals(other.angle);
		}
		return false;
	}
	...
	public static SwerveModuleState optimize(SwerveModuleState desiredState, Rotation2d currentAngle) {
		var delta = desiredState.angle.rotateBy(currentAngle.inverse());
		if (Math.abs(delta.getDegrees()) > 90.0) {
			return new SwerveModuleState(
					-desiredState.speedMetersPerSecond, desiredState.angle.rotateBy(Rotation2d.fromDegrees(180.0)));
		} else {
