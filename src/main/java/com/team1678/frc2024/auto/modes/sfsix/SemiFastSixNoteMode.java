...
public class SemiFastSixNoteMode extends AutoModeBase {
	...
	Trajectory<TimedState<Pose2dWithMotion>> N2PickupToNearShot;
	...
	public SemiFastSixNoteMode() {
		...
		FarShotToN2Pickup = logTrajectory(s.SF6FarShotToN2Pickup);
		N2PickupToNearShot = logTrajectory(s.SF6N2PickupToNearShot);
	...
	protected void routine() throws AutoModeEndedException {
		...
		if (n2_taken) {
			...
			runAction(new ParallelAction(List.of(
					new SwerveTrajectoryAction(FarShotToN2Pickup, false),
					new SeriesAction(
						new WaitAction(0.2),
			...
			d.overrideHeading(true);
			runAction(new SwerveTrajectoryAction(N2PickupToNearShot, false));
		}
